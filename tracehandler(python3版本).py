...
lock = threading.Lock()
nodeNum = 0
shareQueue = Queue.Queue(maxsize=0)#构造一个FIFO队列，maxsize可以限制队列的大小。如果队列的大小达到了队列的上限，就会加锁，加入就会阻塞，直到队列的内容被消费掉。maxsize的值小于等于0，那么队列的尺寸就是无限制的    在第二行有个#python2

def waitNodePort(UISocket, port):
    #print 'line15'
...
def startService():
    ...
    while True:
        ...
        port = UISocket.recv(1024)#收了chuan字符串
        #print 'line57(3)',port
        if (port == '1024'):
            #print 'this is 1024'
            serverSocket.close()
        ...
        t = threading.Thread(target=waitNodePort, args=(UISocket, port))
        t.start()
        #print 'line 57'
def put_log(uwan):
    #print 'line 64'
