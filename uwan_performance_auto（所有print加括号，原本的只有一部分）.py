...
class UWANPerformance(object):
    ...
    def load_log(self, path):
        ...
        loglist = []
        logfile = open(path)
        #next(logfile)#这种=========================Wed Apr  1 15:55:52 2020可能有很多行，要全部筛选，跳过一行是不够的
        for line in logfile:
            line = line.strip('\n')
            if len(line) == 0: # 删除空行
                break
    ...
    def read_log_file(self):
        self.filenames = os.listdir(self.path)#path = $PROJ_DIR/tracefile/
        #print("path: ",path) ('path: ', './logs')
        for filename in self.filenames:
            if len(filename.split('.')) == 2 and filename.split('.')[1] == 'log'and filename.split('.')[0] != 'myeasylog' and filename.split('.')[0] != 'allNodes':
                #print ("slect :",filename)
                self.node_num += 1
                logpath = self.path + '/'+filename
                #print ("logpath :",logpath)
                size = os.path.getsize(logpath)
                if size:#首先要不是空文件，才去分析
...
#    print path
    ...
    with open("result.json", "w") as f:#..在默认工程文件夹那。。
